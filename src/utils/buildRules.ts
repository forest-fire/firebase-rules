...
import { map, reduce, filter, set, pipe, toPairs, flip, concat, join } from "lodash/fp";
...
const filterInvalidRuleKeys = filter((entry) => validRulesKeys.indexOf(entry[0]) !== -1);
...
const parsePathRules = (pathArr) =>
  pipe([toPairs, map((entry) => [parseRuleKey(entry[0]), _parseRuleValue(pathArr)(entry[1])]), filterInvalidRuleKeys]);
...
export const _toParsedRules = reduce(
  (acc, pathRules) =>
    reduce((acc, entry) => set(["rules"].concat(pathRules[0], entry[0]), entry[1], acc), acc, pathRules[1]),
  { rules: {} }
);
